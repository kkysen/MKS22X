 ...
 */
public class GenericHeap<E extends Comparable<? super E>> extends AbstractQueue<E> implements Cloneable {
    ...
    public GenericHeap(final int initialCapacity, final Order order) {
        this.min = order == Order.MIN ? 1 : -1;
        elements = (E[]) new Comparable[initialCapacity + 1];
    }
    
    public GenericHeap(final Order order) {
        this(10, order);
    }
    ...
    private GenericHeap(final GenericHeap<E> clone) {
        min = clone.min;
        size = clone.size;
    ...
    @Override
    public GenericHeap<E> clone() {
        return new GenericHeap<>(this);
    }
    ...
    public static <T extends Comparable<? super T>> void heapSort(final T[] a) {
        final GenericHeap<T> heap = new GenericHeap<>(a.length, Order.MIN);
        for (final T e : a) {
            heap.add(e);
        }
